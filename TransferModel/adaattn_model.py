...
class Transformer(nn.Module):
    ...
    def forward(self, content4_1, style4_1, content5_1, style5_1,
        ...
        print('='*100)
        print(attn_adain_4_1_result.shape, attn_adain_5_1_result.shape)
        print('='*100)
...
def resize_to_even(image, target_size):
    ...
    # Kiểm tra chiều nào nhỏ hơn để resize về 512
    if width < height:
        # Resize chiều rộng về 512 và tính chiều cao sao cho giữ nguyên tỉ lệ và chiều cao là bội của 32
        new_width = target_size
        new_height = int((height / width) * new_width)
    else:
        # Resize chiều cao về 512 và tính chiều rộng sao cho giữ nguyên tỉ lệ và chiều rộng là bội của 32
        new_height = target_size
        new_width = int((width / height) * new_height)

    # Điều chỉnh chiều cao hoặc chiều rộng sao cho là bội của 32
    if new_height % 32 != 0:
        new_height = (new_height // 32) * 32  # Làm tròn xuống bội của 32
    
    if new_width % 32 != 0:
        new_width = (new_width // 32) * 32  # Làm tròn xuống bội của 32

    return image.resize((new_width, new_height))

def adaattn_transfer(content_img, style_img):
    model = AdaAttNModel()

    # Resize và chuyển đổi ảnh sao cho chiều nhỏ hơn là 512, chiều còn lại giữ tỉ lệ và là bội của 32
    content_img = resize_to_even(content_img, target_size=512)
    style_img = resize_to_even(style_img, target_size=512)
    ...
    # Thực hiện chuyển giao phong cách
    result = model.forward(content_img.unsqueeze(0), style_img.unsqueeze(0))[0]
